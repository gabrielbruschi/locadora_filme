...
private: void ListarDtGridViewIndisponivel() {
	cmd = gcnew MySqlCommand("SELECT LOCADORADB.LOCACAO.LOC_ID, LOCADORADB.CLIENTE.NOME, LOCADORADB.CLIENTE.CPF, LOCADORADB.FILME.FIL_ID, LOCADORADB.FILME.TITULO FROM LOCADORADB.LOCACAO INNER JOIN LOCADORADB.CLIENTE INNER JOIN LOCADORADB.FILME  INNER JOIN LOCADORADB.STATUS ON LOCADORADB.LOCACAO.FIL_ID = LOCADORADB.STATUS.FIL_ID AND LOCADORADB.STATUS.DISPONIVEL = 'NAO' AND LOCADORADB.FILME.FIL_ID = LOCADORADB.STATUS.FIL_ID AND LOCADORADB.LOCACAO.CLI_ID = LOCADORADB.CLIENTE.CLI_ID ", con);
