...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->tstId->Click += gcnew System::EventHandler(this, &MainForm::tstId_Click);
			// 
			// tsbBuscar
			// 
...
private: void ListarDtGridViewDisponivel() {
	cmd = gcnew MySqlCommand("SELECT LOCADORADB.FILME.FIL_ID, LOCADORADB.FILME.TITULO, LOCADORADB.FILME.DIRETOR, LOCADORADB.FILME.GENERO, LOCADORADB.FILME.ANO FROM LOCADORADB.FILME INNER JOIN LOCADORADB.STATUS ON LOCADORADB.FILME.FIL_ID = LOCADORADB.STATUS.FIL_ID AND LOCADORADB.STATUS.DISPONIVEL = 'SIM' ;", con);
...
private: void ListarDtGridViewTitulo() {
	cmd = gcnew MySqlCommand("SELECT LOCADORADB.FILME.FIL_ID, LOCADORADB.FILME.TITULO, LOCADORADB.FILME.DIRETOR, LOCADORADB.FILME.GENERO, LOCADORADB.FILME.ANO FROM LOCADORADB.FILME INNER JOIN LOCADORADB.STATUS ON LOCADORADB.FILME.FIL_ID = LOCADORADB.STATUS.FIL_ID AND (LOCADORADB.FILME.TITULO = '" + this->tstId->Text + "' OR LOCADORADB.FILME.FIL_ID = '" + this->tstId->Text + "') ;", con);
...
private: void ListarDtGridViewIndisponivel() {
	cmd = gcnew MySqlCommand("SELECT DISTINCT LOCADORADB.LOCACAO.LOC_ID, LOCADORADB.CLIENTE.NOME, LOCADORADB.CLIENTE.CPF, LOCADORADB.FILME.FIL_ID, LOCADORADB.FILME.TITULO FROM LOCADORADB.LOCACAO INNER JOIN LOCADORADB.CLIENTE INNER JOIN LOCADORADB.FILME INNER JOIN LOCADORADB.DEVOLUCAO  INNER JOIN LOCADORADB.STATUS ON LOCADORADB.LOCACAO.FIL_ID = LOCADORADB.STATUS.FIL_ID AND LOCADORADB.STATUS.DISPONIVEL = 'NAO' AND LOCADORADB.FILME.FIL_ID = LOCADORADB.STATUS.FIL_ID AND LOCADORADB.LOCACAO.CLI_ID = LOCADORADB.CLIENTE.CLI_ID ", con);
...
private: System::Void tstId_Click(System::Object^  sender, System::EventArgs^  e) {
}
};
}
